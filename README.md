...
| 5 | Update Buku | Mencoba memperbarui data buku melalui form edit. | 1. Login sebagai user.<br>2. Klik menu `Tambah Buku`, isi form lalu klik `Simpan Buku`.<br>3. Pada halaman detail, klik tombol `Edit Buku`.<br>4. Ubah salah satu field lalu klik `Perbarui Buku`. | 1. Redirect kembali ke `/books` (dashboard).<br>2. Muncul flash error `Buku gagal diperbarui.`.<br>3. Data buku tidak berubah (judul lama tetap muncul, judul baru tidak muncul). |
